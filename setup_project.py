...
def _build_git_hooks(f, info):
    """git hooks + start.sh。"""
    _add = lambda p, c, x=False: f.__setitem__(p, (c.strip() + "\n", x))

    # ── pre-commit hook ──
    _add(".githooks/pre-commit", r"""#!/bin/bash
...
""", True)
    ...
    # ── pre-push hook ──
    _add(".githooks/pre-push", r"""#!/bin/bash
...
""", True)
    ...
    # ── commit-msg hook ──
    _add(".githooks/commit-msg", r"""#!/bin/bash
...
""", True)
    ...
    # ── start.sh — 角色启动脚本 ──
    _eng_pattern = "乔峰|黄蓉|张三丰|杨过" if info["has_hardware"] else "乔峰|黄蓉|张三丰"
    _review_pattern = "王重阳|一灯大师|郭靖" if info["has_hardware"] else "王重阳|一灯大师"
    _all_roles = "乔峰 黄蓉 张三丰 杨过 王重阳 一灯大师 郭靖" if info["has_hardware"] else "乔峰 黄蓉 张三丰 王重阳 一灯大师"

    _add("start.sh", f"""#!/bin/bash
# 启动角色
# 用法：./start.sh 乔峰
#       ./start.sh 王重阳
set -e
...
def main():
  ...
  启动各角色（工程师自动模式，架构师/审查员正常模式）：
    ...
    {'硬件：    cd ~/Projects/' + d + ' && ./start.sh 杨过' if info['has_hardware'] else ''}
    安全审查：cd ~/Projects/{d} && ./start.sh 一灯大师
    {'嵌入式安全：cd ~/Projects/' + d + ' && ./start.sh 郭靖' if info['has_hardware'] else ''}

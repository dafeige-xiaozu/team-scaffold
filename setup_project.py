...
### 阅读与理解
- 阅读所有项目文档：CLAUDE.md、ARCHITECT.md、SESSION.md、DECISIONS.md
- 阅读各角色状态：`*/STATUS.md`
...
case "$STATUS_KEY" in
    ...
    boss)         echo "📋 请先读: ARCHITECT.md 或 SESSION.md" ;;
    *)            echo "首次启动，无历史状态" ;;
esac
...
description: "架构师 plan mode 约束 — 王重阳专用"
globs: "**"
---
...
## 王重阳的工作模式
王重阳默认工作在 plan mode，是只读规划角色。

## 王重阳可以做
- 阅读任何项目文件（文档、代码、配置、状态）
- 查看 git log、git diff、团队状态、决策记录
- 进行技术方案设计和任务拆解
- 输出工程提示词给开发/联调/审查角色
- 做角色调度和边界裁定
- 指定验证标准
...
## 王重阳不可以做
- 不写业务代码、不改配置文件
- 不执行 Bash 命令（git commit、build、test、deploy 等）
- 不 spawn 子 Agent
- 不替代开发角色（黄蓉/乔峰/杨过）承担实现工作
- 不替代张三丰做部署和联调
- 不替代一灯大师/郭靖做安全审查

## 角色分层原则
- **规划与调度层**：王重阳 — 技术拆解、提示词设计、任务编排、边界裁定
- **功能实现层**：黄蓉、乔峰、杨过 — 各自目录的业务代码，承担最终实现责任
- **环境与联调层**：张三丰 — 部署、环境搭建、全链路联调、最小必要修复
- **审查与风控层**：一灯大师、郭靖 — 各自方向的安全审查
""")
...
## 适用角色
本规则适用于乔峰（后端工程师）。
张三丰（联调负责人）为联调目的修改 backend/ 文件时，遵循 infra-deploy.md 中的跨目录修改权限，不受本规则的"拒绝"条款约束。
""")
...
## 适用角色
本规则适用于黄蓉（前端工程师）。
张三丰（联调负责人）为联调目的修改 frontend/ 文件时，遵循 infra-deploy.md 中的跨目录修改权限，不受本规则的"拒绝"条款约束。
""")
